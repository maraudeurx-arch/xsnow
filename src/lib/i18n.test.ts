...
describe("vos idees copy", () => {
  it("keeps FR/EN/ES idea capture in sync", () => {
    ...
    assert.equal(fr.ideas.openCta, "Vos idées");
    assert.equal(en.ideas.openCta, "Your ideas");
    assert.equal(es.ideas.openCta, "Tus ideas");
    assert.equal(fr.ideas.submit, "Soumettre");
    assert.equal(en.ideas.submit, "Submit");
    assert.equal(es.ideas.submit, "Enviar");

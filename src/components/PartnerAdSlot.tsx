...
export function PartnerAdSlot({ slot }: { slot: PartnerSlotId }) {
  ...
  return (
    <aside
      ...
      className="flex min-h-0 flex-1 flex-col rounded-lg border border-dashed border-gold/35 bg-white/[0.03] px-2 py-1.5 text-left"
    >
      ...
      {adsense ? (
        ...
        <div
          className={`mt-1 flex ${SLOT_MIN_H[slot]} min-h-0 flex-1 flex-col items-center justify-center rounded-md border border-white/10 bg-night/40 px-2 py-1.5 text-center`}
        >

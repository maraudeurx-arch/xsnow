...
`NEXT_PUBLIC_ADS_ENABLED` / `NEXT_PUBLIC_ADS_PROVIDER` : espaces **Commandité / Espace partenaire** (top + mid + bottom) sous les manchettes (pas de fausses news). Défaut : placeholders. `adsense` + `NEXT_PUBLIC_ADSENSE_CLIENT` / `NEXT_PUBLIC_ADSENSE_SLOT_NEWS_*` pour brancher un réseau plus tard. `false` ou `provider=none` les cache.
